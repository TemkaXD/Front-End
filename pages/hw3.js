  ...
  return (
    <div className="bg-gray-900 text-white">
      <div className="min-h-screen flex">
        <div className="bg-gray-800 w-20 p-4 flex flex-col rounded-3xl items-center space-y-8 mt-4 mb-4 ml-4">
          ...
          <div className="w-[98%] h-[96%] flex justify-between items-end">
            <div className="w-2/3 h-full rounded-3xl">
              ...
              <div className="w-full h-64 bg-slate-900 rounded-3xl flex items-end justify-center relative">
                ...
                <div className="w-11/12 h-2/3 mb-8 grid grid-cols-6 gap-4">
                  {['6:00 AM', '9:00 AM', '12:00 PM', '3:00 PM', '6:00 PM', '9:00 PM'].map((time, index) => (
                    <div key={index} className="flex flex-col items-center">
                      <h1 className="text-slate-400 font-semibold">{time}</h1>
                      <Image 
                        src={index % 2 === 0 ? "https://png.pngtree.com/png-vector/20230414/ourmid/pngtree-sun-orange-three-dimensional-illustration-png-image_6694186.png" : "https://cdn-icons-png.flaticon.com/128/1163/1163624.png"} 
                        alt="Weather Icon" 
                        className="mt-4 w-2/3" 
                        width={32} 
                        height={32} 
                      />
                      <h1 className="text-white font-bold text-3xl">{[2, 10, 14, 16, 12, 6][index]}°</h1>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="w-full h-60 bg-slate-900 rounded-3xl mt-4 relative flex flex-col justify-end">
                ...
                <div className="w-full h-2/3 grid grid-cols-4">
                  {['Real Feel', 'Wind', 'Chance of Rain', 'UV Index'].map((label, index) => (
                    <div key={index} className="relative flex flex-col items-center justify-center">
                      <h1 className="text-slate-400 text-md font-semibold">{label}</h1>
                      <h1 className="text-white font-bold text-3xl mb-8">{[6, '5.4 km/h', '0%', 3][index]}</h1>
                    </div>
                  ))}
                </div>
              </div>
